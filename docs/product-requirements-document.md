...
| 2026-06-04 | v1.6.2 | 团队 | **需关注事项**仅保留计划逾期与长期未开始；控制点列表 **GITC/ITAC 标签切换**；成员管理 **Specialist team staff** 统一区块（Lead 仅见本组 Staff）；移除独立 Staff 页与阻塞链预览组件 |
...
| 需关注事项 | **计划逾期**、**长期未开始**（见 §13.3）；无事项时不展示该区块 |
| 控制点列表 | **GITC / ITAC** 标签切换；列：类型、控制点、负责人、负责组、状态、进度%、阶段 |
| 只读抽屉 | **双状态**（§13.9）、测试摘要、材料、阻塞前置；「去工作台」「去看板」 |

**本期不做**：进度页内编辑材料、拖拽、导出报告、跨项目汇总、阻塞链迷你依赖图。
...
| 类型 | 口径 |
|------|------|
| **计划逾期** | `due < 今天` 且 `progressStatus !== completed` |
| **长期未开始** | 看板 `status === todo` 且 `progressStatus === not_started`，并满足：项目开始 ≥14 天仍未启动，**或** 距 `due` ≤7 天仍未启动 |

点击条目等同列表选中并打开抽屉。KPI「逾期/阻塞」口径仍含 `blocked`（见 §13.4），与需关注区展示范围不同。

### 13.4 控制点列表标签（v1.6.2）

- 列表标题旁展示 **GITC · N** / **ITAC · N**（及其他类型，若有）切换按钮  
- 每次仅渲染当前标签对应控制点表格  
...
### 13.6 progressStatus 展示文案（全中文 · 已冻结）
...
| **底稿状态（工作台）** | `progressStatus` | 未开始 / 测试中 / 资料已获取 / …（§13.6） |
