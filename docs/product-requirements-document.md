...
| 2026-07-01 | v1.7.8 | 团队 | **P0 黑客松 demo 改造**：主叙事切换为 **AI 驱动的财审 x ITA 协作驾驶舱**；组合驾驶舱新增可收起/展开的 **AI 审计指挥入口**（deterministic mock 自然语言查询）；项目列表默认收起；项目卡展示 **Audit 关注点 → ITA 测试点 → 底稿状态 → 财审影响**；演示种子 v3 补充财报科目、业务循环、审计断言、IT 依赖系统；固定银行年审 D-5 黄金场景（DS-901 / DS-902 / DS-905）。工作台 AI 生成、上传材料、抽样 Excel 暂不改代码。 |
| 2026-07-01 | v1.7.9 | 团队 | **P1/P1.1 EM 交付风险闭环**：默认 demo 视角改为 **EM · Manager**；侧栏拆分 **全部项目** 与 **团队管理**；团队管理页强瘦身为 **交付风险摘要 → 资源调度驾驶舱 → AI 审计入口 → 明细入口**；新增 deterministic **人到人资源调度建议** 与测试点定位跳转；进度看板支持 `controlId` focus 提示；演示种子 v4 强化 staff1 高负荷、staff3 可承接；项目列表 / 报告日日历 / 资源明细表回到已有页面或后续路线，工作台代码仍不修改。 |
...
| 查询 | JQL | **v1.7.8 demo**：自然语言 mock 查询，围绕报告日、财审循环、IT 控制阻塞返回稳定结果 |
...
3. 主责团队仅 **ITA、Audit**
4. 不接真实大模型 API；AI 能力先使用 deterministic mock，保证视频录制稳定
...
**增强（v1.7.8，黑客松 P0 demo）**

1. 管理视角首屏新增 **AI 审计指挥入口**，支持自然语言 mock 查询与推荐问题一键触发
2. AI 结果输出风险摘要、命中项目 / 测试点、财审影响说明、建议优先行动，并可跳转对应项目进度看板
3. 演示数据补充财审关联标签：财报科目、业务循环、审计断言、IT 依赖系统
4. 项目卡片展示「Audit 关注点 → ITA 测试点 → 底稿状态 → 财审影响」链路
5. 工作台相关 AI 生成 / 上传材料 / 抽样 Excel 能力仅保留为后续 Pending，不纳入本轮代码改动

**增强（v1.7.9，黑客松 P1/P1.1 demo）**

1. Demo 默认进入 **EM · Manager**，主叙事升级为「报告日前交付风险 + 人力负荷错配 + 可执行资源调度建议」
2. 侧栏拆分 **全部项目** 与 **团队管理**：全部项目只做项目浏览，团队管理只做 EM 决策闭环
3. 团队管理页强瘦身：仅保留 **交付风险摘要 → 资源调度驾驶舱 → AI 审计入口 → 明细入口**，项目列表 / 日历 / 资源明细不再重复展示
4. 资源调度驾驶舱新增 deterministic 人到人建议：从高负荷人员协调给低负荷人员，并跳转进度看板定位对应 DS 测试点
5. 工作台相关 AI 生成 / 上传材料解析 / 抽样 Excel 继续 Pending，不修改工作台代码
...
| 核心价值 | AI 驱动的财审 x ITA 协作驾驶舱：用财审报告日、业务循环与 IT 控制风险串起项目创建、跨 Specialist 协作、工作台维护控制点、测试执行与进度可视 |
...
DeepSleep
├── 全部项目（v1.7.9 · 独立入口）
│   └── 模糊搜索、排序、牵头团队/项目类型筛选、类型皮肤
├── 团队管理（v1.7.9 · EM 交付风险闭环）
│   ├── 角色视角 · View as：EP / EM / IC / Staff / Lead（不含全部项目）
│   └── 交付风险摘要、资源调度驾驶舱、AI 审计指挥入口、明细入口
├── 项目类型（v1.7 · 展示）
│   ├── 五类档案卡、查看示例、以此类型创建
├── 项目创建 / 详情 / 成员管理（v1.5）
...
## 七、功能需求 · 全部项目页（v1.5 列表 · v1.7.9 独立入口）

全局侧栏入口 **全部项目**（`EngagementHomePage`）。该页面恢复为纯项目浏览页，不承载团队管理、AI 指挥入口或资源调度模块。

| 入口 | 页面行为 |
|------|----------|
| **全部项目** | 原项目列表：卡片点击进入 **项目概览**；支持搜索、筛选、排序 |
| **团队管理** | 独立侧栏入口，见 §7.5；不在全部项目页内用 View as 切换 |

**Demo 默认**：首次进入（无当前项目）打开 **团队管理 · EM · Manager**，服务 P1 评委叙事；用户可从侧栏进入 **全部项目** 查看完整项目清单。
...
**全部项目**页提供牵头团队 / 项目类型筛选，并额外提供 §7.4 排序。
...
## 七点五、功能需求 · 团队管理页（v1.7 · v1.7.9）

> 团队管理是 P1/P1.1 的 demo 主页面：面向 EM / EP / IC 快速判断报告日前交付风险、人力负荷错配，并给出可执行但不自动改派的资源调度建议。

### 入口与布局（v1.7.9）

- 全局侧栏：**全部项目 → 团队管理 → 新建项目**
- 团队管理页顶固定 **角色视角 · View as**，选项仅含 **EP / EM / IC / Staff / ITA Lead / Tax Lead**，不再含「全部项目」
- Demo 默认（清 session 后）为 **EM · Manager**
- **EP / EM / IC / Staff / ITA·Tax Lead** 共用精简后的 `ManagementCommandBody.jsx` 壳层
- 关注级别 RAG 与进度看板 **workspaceStatus / overdue token 同源**（`portfolioVisualTokens.js`）

**团队管理页内区块顺序（自上而下 · v1.7.9）**

1. **压缩版角色栏**：保留 View as 和当前演示邮箱，说明文字弱化为一行
2. **交付风险摘要** KPI **三格**：需立即关注 / 需重点关注 / 30 天内报告；保留报告日集中提示
3. **资源调度驾驶舱**（`ResourceDispatchPanel`）：展示 3 条人到人建议，按钮仅「查看测试点」
4. **AI 审计指挥入口**（`AiAuditCommandPanel`）：默认收起，可展开自然语言查询；返回风险摘要、命中测试点、财审影响、建议行动与进度看板跳转
5. **明细入口条**：提供「查看全部项目」和「打开最高风险项目进度」；明细回到已有页面，不在团队管理页重复陈列

**v1.7.7 移除**：独立「优先关注」区块（排名并入项目卡片 `#1–#N`）、**报告日时间轴**（`ReportTimelineStrip`）、**指标说明**（`CommandMetricsLegend`）、EP/EM 底部「下辖 EM / 现场团队」折叠区（信息并入资源分配）

**v1.7.9 强瘦身策略**：团队管理页不再渲染项目列表卡片、项目筛选 toolbar、报告日日历、报告日预警列表、资源分配明细表。上述信息分别通过 **全部项目** 与 **进度看板** 进入，确保团队管理首屏聚焦「交付风险 → 调度建议 → AI 解释」。

### AI 审计指挥入口（v1.7.8 · deterministic mock）

**目标**：把 demo 叙事从「项目看板」升级为「AI 驱动的财审 x ITA 协作驾驶舱」，让评委在 5 分钟内看到财审报告日压力如何被 IT 控制测试进度阻塞，以及系统如何给出可执行优先级。
...
| 字段 | 说明 |
|------|------|
| 风险摘要 | 面向 EP / EM 的一句话结论，聚焦报告日、业务循环与阻塞风险 |
| 命中的项目 / 测试点 | 返回项目、控制点 ID、标题、逾期 / 预警状态 |
| 财审影响说明 | 映射至财报科目、业务循环、审计断言与 IT 依赖系统 |
| 建议优先行动 | 明确先跟进哪个 DS 编号、谁负责、为什么影响报告日 |
| 跳转 | 主按钮进入对应项目 **进度看板** |

**关键词命中规则（demo 稳定优先）**：输入含「收入」命中收入循环；含「报告日 / 下周 / 逾期 / 阻塞」命中报告日阻塞；含「资源 / 工作负荷 / 谁」命中资源负荷。未命中时返回黄金 demo 场景。

**实现边界**：不接真实大模型 API；不写入 localStorage；不修改 `WorkspacePage.jsx` 或 `workspaceProgressService.js`。

### 资源调度驾驶舱（v1.7.9 · deterministic mock）

**目标**：当 EM 同时管理多个报告日密集项目时，系统直接指出谁高负荷、谁可承接、应优先查看哪个测试点。

**展示内容：**

| 字段 | 说明 |
|------|------|
| 资源健康摘要 | 现场成员数、高负荷人数、可承接人数、逾期测试点数 |
| 调度建议 | 3 条「从高负荷人员 → 协调给低负荷人员」建议 |
| 测试点定位 | 每条建议展示 DS 编号、标题、客户、报告日风险原因 |
| 操作 | **查看测试点**：进入项目进度看板并自动定位对应 DS 测试点 |

**规则**：

- 仅 EP / EM / IC 视角显示；Staff / Lead 视角不显示资源调度模块
- 建议只读，不修改 owner，不创建任务，不写工作台
- 优先选择未完成、逾期或报告日在 14 天内的测试点；演示重点为银行年审 DS-901 / DS-902 / DS-905 及 staff1 高负荷场景
- 找不到完整人到人建议时，显示保守建议：升级协调或优先查看高风险测试点

**进度看板跳转**：`onOpenProgress` 支持对象参数 `{ projectId, controlId, ownerEmail? }`；打开进度看板后展示提示条 `调度关注：DS-xxx · 已定位到建议跟进测试点`。
...
| 身份 | 默认关注 |
|------|----------|
| **EP · Partner** | Portfolio Oversight：交付风险摘要 + 资源调度 + AI 解释 |
| **EM · Manager**（**Demo 默认**） | 团队管理主叙事：报告日前交付风险与人力重分配建议 |
| IC · In-charge | 组内交付风险摘要；可查看资源调度建议 |
| Staff | 个人参与项目风险摘要；不显示资源调度模块 |
| ITA Lead / Tax Lead | 协作组项目风险摘要；不显示资源调度模块 |

选择身份后写入 `sessionStorage`，与进度看板共用；storage key 升级避免旧 session 停留在「全部项目」视角。

### 项目卡（只读 · v1.7.8 保留能力）

项目卡仍作为可复用组件保留，但 **v1.7.9 团队管理页不默认渲染项目卡列表**。若后续在项目明细或其他组合页复用，每张卡展示：类型皮肤、客户/项目名、Audit-led/ITA-led、迷你四态条、逾期数、报告日、协作组 pills、**Audit 关注点 → ITA 测试点 → 底稿状态 → 财审影响**链路。

操作：**进度看板**（主） / **概览**（次）。进入进度看板时携带当前 View as 并应用默认筛选。

### 搜索与筛选

团队管理页不再提供项目搜索 / 筛选 toolbar；完整搜索、牵头团队筛选、项目类型筛选统一回到 **全部项目** 页面。
...
View as = **Staff** 时，团队管理页展示个人参与项目的交付风险摘要 + AI 入口 + 明细入口；不显示资源调度驾驶舱，不渲染项目卡列表。

**展示范围**：仅「有成员身份 **或** 有指派测试点（`task.owner`）」的项目。

**明细入口**：进入 **全部项目** 或 **进度看板** 查看完整项目 / 测试点明细。

历史 Staff 个人工作全景组件能力（参与项目列表、当前主攻、个人测试点三态）作为后续可恢复能力保留，不进入 v1.7.9 团队管理首屏。

### IC / EM 模式 · 团队负荷 Rollup（v1.7.2 · v1.7.9 调度优先）

View as = **IC** 或 **EM** 时，团队管理页优先展示 **资源调度驾驶舱**，不再展示完整 `PersonWorkloadTable` 明细表。
...
`PersonWorkloadTable` 与 `ResourceAllocationSection` 作为后续“资源明细页 / 高级明细”能力保留；v1.7.9 团队管理页只展示摘要统计与 3 条调度建议。

实现：`personWorkloadUtils.js` + `PersonWorkloadTable.jsx` + `teamRollupUtils.js` + `TeamRollupCommandView.jsx` + `ManagementCommandBody.jsx`。

### EP 模式 · Engagement Portfolio Oversight（v1.7.4 · v1.7.9）

View as = **EP** 时，切换为 **Portfolio Oversight 页**（`EpCommandView` + `ManagementCommandBody`）。

**交付风险摘要三格**（v1.7.9）：需立即关注 / 需重点关注 / 30 天内报告；附组合摘要（项目数 · 下辖 EM 数）。

**资源调度驾驶舱**（v1.7.9）：按下辖团队内的高负荷 / 可承接人员生成建议，点击定位测试点。

**AI 审计指挥入口**（v1.7.8 / v1.7.9）：默认收起；展开后可用自然语言查看「报告日前财审关键循环被 IT 控制阻塞」「报告日前如何重新分配 staff 工作？」等 mock 结论。

**明细入口**：项目列表、项目概览、完整进度与资源明细不在 EP/EM 团队管理页重复展示，通过 **全部项目** 与 **进度看板** drill down。

实现：`epPortfolioUtils.js` + `PortfolioKpiSection.jsx` + `ResourceDispatchPanel.jsx` + `resourceDispatchUtils.js` + `AiAuditCommandPanel.jsx` + `ManagementCommandBody.jsx`。
...
## 七点七、演示种子项目（v1.7 · v1.7.9）

首次启动幂等注入（`deepsleep-demo-seed-v1`，**版本 4** 覆盖演示项目字段并刷新演示测试点）：
...
**财审关联标签（v1.7.8）**：演示测试点补充 `financialStatementAccounts`、`businessCycles`、`auditAssertions`、`itDependencySystems`、`financialAuditImpact` 等前端字段。覆盖科目包括收入、货币资金、总账、采购付款、固定资产、薪酬；覆盖循环包括收入循环、资金循环、采购付款循环、总账关账循环；覆盖断言包括完整性、准确性、发生、截止、权限控制；IT 依赖系统包括核心银行系统、总账系统、IAM、变更管理平台等。

**黄金 demo 场景（v1.7.8）**：某银行年审报告日 **D-5**，收入 / 资金相关 ITAC / GITC 逾期或预警，AI 建议优先跟进 **DS-901、DS-902、DS-905**。

**资源调度 demo 场景（v1.7.9）**：`manager.uat@firm.com` 所辖项目中，`staff1.uat@firm.com` 明显高负荷，包含多个逾期 / D-7 窗口测试点；`staff3.uat@firm.com` 保持低负荷，可作为建议承接人。资源调度驾驶舱优先提示 **DS-901 / DS-902 / DS-905** 等报告日前关键测试点，但不自动改派 owner。

**原则**：工作台页面与写入逻辑 **不修改**；种子仅写 `projects` / `tasks` / 可选 workspace progress。升级种子版本后需清 `localStorage` 中 `deepsleep-demo-seed-v1` 或首次访问自动重灌。

---

## 七点八、资源调度与资源分配（v1.7.9）

**团队管理页主展示为资源调度驾驶舱，而不是完整资源分配明细表。**

| 视角 | 团队管理页展示 | 说明 |
|------|----------------|------|
| **EP** | 资源调度驾驶舱 | 在下辖团队内识别高负荷 / 可承接人员与高风险测试点 |
| **EM** | 资源调度驾驶舱 | P1 demo 主叙事：报告日前 staff 工作重分配建议 |
| **IC** | 资源调度驾驶舱 | 组内 IC/Staff 维度建议 |
| **Staff** | 不显示资源调度 | 仅展示个人相关风险摘要 + AI + 明细入口 |
| **ITA/Tax Lead** | 不显示资源调度 | 仅展示协作组风险摘要 + AI + 明细入口 |

**调度建议口径：**

- 高负荷候选：`load-high` 优先，其次逾期数、控制点数
- 可承接候选：`load-low` 优先，其次控制点数少、逾期少
- 测试点候选：未完成、逾期或项目报告日在 14 天内，优先 D-7 / 逾期测试点
- 建议只读：不改 owner，不写任务，不写工作台
- 主按钮：**查看测试点**，跳转进度看板并定位 DS 测试点

**完整资源分配明细（v1.7.7 能力保留，v1.7.9 不在团队管理页首屏渲染）：**
...
- [x] **AI 审计指挥入口**（`AiAuditCommandPanel`）：自然语言 deterministic mock 查询；默认收起、可展开
- [x] 固定三条推荐问题：报告日阻塞、收入循环 IT 风险、资源负荷影响
- [x] AI 结果展示风险摘要、命中项目 / 测试点、财审影响、建议优先行动与进度看板跳转
- [x] **财审关联标签**（`financialAuditContext.js`）：财报科目、业务循环、审计断言、IT 依赖系统、财审影响
- [x] 项目卡片展示 **Audit 关注点 → ITA 测试点 → 底稿状态 → 财审影响** 链路
- [x] **演示种子 v3**：固定银行年审 D-5 黄金场景，优先跟进 DS-901 / DS-902 / DS-905
- [x] 日历项目按钮固定在日期格底部并限制在格内；项目列表默认收起 / 展开
- [x] 工作台相关 AI 生成、材料上传、抽样 Excel、Design / Implementation 逻辑保持 Pending，未修改工作台代码

**v1.7.9**

- [x] 默认 demo 视角改为 **EM · Manager**；View as storage key 升级，避免旧 session 停留在「全部项目」
- [x] 侧栏拆分 **全部项目** 与 **团队管理**；全部项目页不再显示角色视角下拉 / AI / 调度模块
- [x] 团队管理页强瘦身：**交付风险摘要 → 资源调度驾驶舱 → AI 审计入口 → 明细入口**；不再渲染项目列表卡片、筛选 toolbar、报告日日历、报告日预警、资源明细表
- [x] **资源调度驾驶舱**（`ResourceDispatchPanel` + `resourceDispatchUtils`）：3 条 deterministic 人到人建议，只读不改派
- [x] AI 第三个推荐问题改为 `报告日前如何重新分配 staff 工作？`，AI 结果与调度驾驶舱口径一致
- [x] 进度看板跳转支持 `{ projectId, controlId, ownerEmail? }`，打开后显示 `调度关注：DS-xxx`
- [x] **演示种子 v4**：强化 EM 所辖 staff1 高负荷、staff3 可承接的可信调度场景
- [x] 工作台页面与 `workspaceProgressService.js` 未修改

**v1.7.6（历史能力，v1.7.9 已拆为「全部项目」与「团队管理」）**
...
- 工作台后半段 AI 能力：上传制度 / 会议纪要 → AI 生成底稿草稿
- 抽样 Excel 与 Design / Implementation 逻辑增强
- 接入真实大模型 API 前的权限、脱敏、审计日志与稳定性设计

### 15.3 本期仍不做

- 跨项目个人工作台、进度页拖拽/写材料、真实权限（Specialist 只看本组）
- 显式 dependencies 手工连边（后期 Scope 模板）
- 真实大模型 API、真实邮件、Tax/Cyber 主责团队
- 工作台内 AI 生成、上传材料解析、抽样 Excel 自动化
...
### C. 演示脚本（5 分钟以内 · v1.7.9）

1. **开场定位**：进入 **团队管理 → EM · Manager**，说明 DeepSleep 帮 EM 在报告日前同时看清交付风险、资源错配和 ITA 测试点阻塞。
2. **交付风险摘要**：展示 **需立即关注 / 需重点关注 / 30 天内报告** 三格 KPI，点出某银行年审报告日 **D-5** 且收入 / 资金相关 IT 控制仍有逾期。
3. **资源调度驾驶舱**：讲 3 条人到人建议，重点说明 `staff1.uat@firm.com` 高负荷、`staff3.uat@firm.com` 可承接，系统建议优先查看 **DS-901 / DS-902 / DS-905**。
4. **AI 解释与追问**：展开 **AI 审计指挥入口**，点击 `报告日前如何重新分配 staff 工作？` 或 `下周报告日前，哪些财审关键循环被 IT 控制阻塞？`，展示风险摘要、财审影响和建议行动。
5. **跳转进度看板**：点击资源建议的 **查看测试点**，进入对应项目进度看板，展示 `调度关注：DS-xxx` 提示和已定位的测试点。
6. **明细与收束**：点击 **查看全部项目** 展示项目列表已独立出来；说明工作台写入层保持不变，本轮 mock AI / deterministic 调度先解决评委可感知的跨项目优先级、报告日风险解释和资源协调闭环。
