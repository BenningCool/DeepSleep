...
│   └── 管理视角（EP/EM/IC/Staff/Lead）：组合 KPI、AI 审计指挥入口、项目列表、资源分配、报告日日历｜预警
├── 项目类型（v1.7 · 展示）
│   ├── 五类档案卡、查看示例、以此类型创建
├── 项目创建 / 详情 / 成员管理（v1.5）
...
**各角色页内区块顺序（自上而下 · v1.7.8）**

1. **组合风险概览** KPI **三格**：需立即关注 / 需重点关注 / 30 天内报告（**已移除**程序执行 KPI 行）
2. **AI 审计指挥入口**（`AiAuditCommandPanel`）：默认收起，可展开自然语言查询；返回风险摘要、命中测试点、财审影响、建议行动与进度看板跳转
3. **项目列表**（`EngagementPortfolioCardList` + `PortfolioListToolbar`）：默认收起；展示当前视角下**全部**相关项目（不按报告月过滤）；搜索 / 牵头团队 / 项目类型 / 排序 / 计数并入列表模块
4. **资源分配**（`ResourceAllocationSection` · 见 §7.7）
5. **报告日日历**（`ReportDateCalendar` · 月份切换）与 **报告日预警**（`ReportDayPanel`）**左右并排**

**v1.7.7 移除**：独立「优先关注」区块（排名并入项目卡片 `#1–#N`）、**报告日时间轴**（`ReportTimelineStrip`）、**指标说明**（`CommandMetricsLegend`）、EP/EM 底部「下辖 EM / 现场团队」折叠区（信息并入资源分配）

**v1.7.8 收起策略**：AI 审计指挥入口与项目列表均默认收起，保证 EP 首屏先看到「报告日风险 + AI 入口 + 摘要」，展开后进入详细项目与测试点 drill down。
...
每张卡展示：类型皮肤、客户/项目名、Audit-led/ITA-led、**迷你四态条**（与列表柱同源）、**逾期数**、**报告日 D-xx**、**协作组 pills**（按测试点 `contributorGroup` 实际有数据的组）、**Audit 关注点 → ITA 测试点 → 底稿状态 → 财审影响**链路。
...
### EP 模式 · Engagement Portfolio Oversight（v1.7.4 · v1.7.8）
...
**AI 审计指挥入口**（v1.7.8）：默认收起；展开后可用自然语言查看「报告日前财审关键循环被 IT 控制阻塞」等 mock 结论。

**项目卡片列表**：默认收起；展开后带 `#1–#N` 优先排名、测试点进度条、报告日/逾期 badge、财审影响链路；操作：**项目进度** / **项目概览**。
...
## 七点七、演示种子项目（v1.7 · v1.7.8）

首次启动幂等注入（`deepsleep-demo-seed-v1`，**版本 3** 覆盖演示项目字段并刷新演示测试点）：
...
### C. 演示脚本（5 分钟以内 · v1.7.8）

1. **开场定位**：进入 **项目 → EP · Partner**，说明 DeepSleep 是 AI 驱动的财审 x ITA 协作驾驶舱，当前是某银行年审报告日 **D-5**。
2. **AI 指挥入口**：展开 **AI 审计指挥入口**，点击 `下周报告日前，哪些财审关键循环被 IT 控制阻塞？`。
3. **讲 AI 结果**：展示风险摘要、命中 DS-901 / DS-902 / DS-905、对应收入 / 资金循环、断言与 IT 依赖系统，强调财审报告日影响。
4. **项目列表 drill down**：展开项目列表，展示项目卡里的 **Audit 关注点 → ITA 测试点 → 底稿状态 → 财审影响** 链路。
5. **跳转进度看板**：从 AI 结果或项目卡点击 **进度看板**，查看测试点进度、逾期与负责人。
6. **收束价值**：回到组合视角，说明工作台写入层保持不变，本轮 mock AI 先解决评委可感知的跨团队优先级与报告日风险解释。
