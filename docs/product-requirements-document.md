...
| **workspaceStatus** | 工作台底稿三态：`not_started` / `in_progress` / `completed`（进度看板环形图与列表主状态） |
...
3. 进度看板阶段条仅展示 6 步；抽屉并排展示 **执行状态（看板）** 与 **底稿状态（工作台）**
...
│   └── Scope 未明确前锁死
└── 进度看板（v1.6 · 只读层）
    ├── KPI、阶段条、需关注事项、控制点列表（GITC/ITAC 标签）
...
- 计划开始日期 / 报告日（如有）
- Scope 状态、成员数量
- **状态概述柱**（v1.6.4）：与进度看板 **状态概述环形图同一数据**（`workspaceStatus` 三态：未开始 / 测试中 / 已完成）；列表为 **横向分段柱**，看板为 **环形图**，仅展现形式不同

### 7.3 状态概述柱（v1.6.4）

与 §13 进度看板「状态概述」共用 `computeWorkspaceStatusBreakdown` 口径：

| 页面 | 展现 | 数据 |
|------|------|------|
| 项目列表卡片 | 横向分段柱（灰 / 蓝 / 绿）+ 三态计数文案 | `workspaceStatus` 三态占比 |
| 进度看板 | 环形图 + 图例 | 同上 |

| 场景 | 展示 |
|------|------|
| `scopeStatus === pending` | 灰空轨 + 文案「待 Scope」 |
| Scope 已生成、有控制点 | 分段柱 + `{total} 控制点` + `未开始 n · 测试中 n · 已完成 n` |
| Scope 已生成、0 控制点 | 灰空轨 +「暂无控制点」 |

**不做**：TOD/TOE 节点柱、与三态无关的单一完成率条。
...
`scopeStatus === "pending"` 时，工作台 / 看板 / 进度看板均不可用；`defined` 后解锁。
...
项目组**全员**可在独立「**进度看板**」Tab **只读**查看当前项目的控制点健康度、逾期/阻塞链，以及 Audit vs Specialist 分组进度。**底稿进度、状态概述、组内成员进度** 均来自 `workspaceProgressService` 工作台 snapshot；看板 `task.status` 仅在抽屉「执行状态」中只读展示。**不需要在进度看板内拖拽或编辑**。

**权限**：本期不做角色级差异——能访问该项目的成员均可查看进度看板 Tab。

### 13.2 页面结构（v1.6.3 · 摘要仪表盘 + 明细）

| 区块 | 内容 |
|------|------|
| 负责组筛选 | 全部 / Audit 主责 / ITA 组 / Tax 组 / FRM 组；可叠加 **工作台三态**筛选（点击环形图图例） |
| **摘要 KPI 四格** | 未开始 / 测试中 / 已完成 / 已逾期（均来自控制点 `workspaceStatus` 与计划完成日；逾期待上游同步时显示占位） |
| **状态概述** | **环形图（三态）**：`workspaceStatus` = 未开始 / 测试中 / 已完成；中心为控制点总数 |
| **近期动态** | 按 `updatedAt` 展示最近保存的测试点（节点进度、TOD/TOE、负责人） |
| **分布卡片** | 控制类型（GITC/ITAC）、**组内成员进度**（§13.11） |
| **视觉语义（v1.6.6–v1.6.8）** | 未开始=灰/琥珀警示、测试中=蓝、已完成=绿；逾期=红色最高优先级，在 KPI「已逾期」、需关注列表、控制点列表行三处同时强调（§13.12） |
| 需关注事项 | **计划逾期**、**长期未开始**（见 §13.3） |
| 控制点列表 | **GITC / ITAC** 标签；列：底稿状态（三态）、节点进度（含 TOD/TOE）、阶段；**不含负责人下拉筛选**（v1.6.8） |
| 只读抽屉 | **双状态**：看板执行状态 + 工作台底稿三态 + 细态 `progressStatus`（§13.10） |

**本期不做**：进度看板内编辑材料、拖拽、导出报告、跨项目汇总、外部图表库依赖、模块级「双状态不一致」统计。
...
- 列表标题旁 **GITC · N** / **ITAC · N** 标签切换  
- **底稿状态**列主展示 `workspaceStatus`（三态）；`progressStatus` 细态以小字补充（被阻塞等）  
- **节点进度**列：`completedNodes/totalNodes`，附 `TOD x/y · TOE x/y`  

### 13.5 摘要 KPI 口径（v1.6.7）
...
KPI 卡片可点击筛选列表（三态 KPI 切换 `workspaceStatus` 筛选；「已逾期」切换逾期-only 筛选）；数据**仅来自工作台 snapshot**，不读取看板执行状态。

**KPI 卡片布局（v1.6.8）**：图标 | 标题+说明 | 大号数字；有计数时启用对应色强调（`has-value`），零计数为 idle 样式。
...
| 列表列名 | 类型 / 控制点 / 负责人 / 负责组 / 状态 / 节点进度 / 阶段 |

### 13.10 抽屉（v1.6.1 · 双状态 · v1.6.3 工作台字段）

进度抽屉展示看板执行状态与工作台 snapshot 对照：

| 展示标签 | 数据来源 | 含义 |
|----------|----------|------|
| **执行状态（看板）** | `task.status` | 看板列位置（待办/进行中/已完成等） |
| **底稿状态（工作台）** | `workspaceStatus` | 未开始 / 测试中 / 已完成（§13.5 三态） |
| **细态** | `progressStatus` | 待复核 / 被阻塞等（§13.6） |

看板已推进但底稿未齐备时展示单行提示（§13.3 不做模块级统计）。抽屉另展示：节点进度（TOD/TOE）、Planning/Review milestones、SPP/纪要/资料计数、`fieldReviewSummary`、测试摘要。提供「去工作台」「去看板」入口。
...
| 项 | 口径 |
|----|------|
| 数据来源 | `getControlProgressSnapshot` → `workspaceStatus` 三态（与状态概述、项目列表柱图一致） |
| Team 标签 | **无**（跟随页顶负责组筛选，卡片内不再切换 Team） |
| 「全部」 | 展示 **Audit + 已启用 ITA/Tax/FRM** 全部 active 成员及其名下控制点（去重） |
| 选定某组 | 仅该组名册成员 + 对应 `contributorGroup` 控制点 |
| 成员行 | 组内全部 active 成员均展示（含 0 控制点）；按控制点总数降序 |
| 柱图 | **填充** = `已完成控制点 / 该成员控制点总数`（仅全完成时满条，绿色）；**辅文**为三态明细（未开始 / 测试中 / 已完成） |
| 未分配 | `owner` 为空时单独一行 |

### 13.12 视觉优先级（v1.6.6–v1.6.8）

面向合伙人/经理的鸟瞰使用场景，状态需“一眼可辨”：

| 层级 | 规则 |
|------|------|
| 三态主色 | `workspaceStatus`：未开始（灰，有值时琥珀警示）/ 测试中（蓝）/ 已完成（绿） |
| KPI 图标 | 四格均使用 SVG 语义图标（时钟/进度弧/对勾/警告三角） |
| 逾期优先级 | 逾期（红）高于其他状态展示；在列表行与关注事项中可覆盖默认中性色背景 |
| KPI「已逾期」强提示（v1.6.8） | 有计划完成日且 `overdue > 0`：8px 深红左边框、红橙渐变底、脉冲外发光、数值放大、右上角 **「需跟进」** 徽章；点击切换逾期-only 列表筛选 |
| KPI「已逾期」idle | 有计划完成日但零逾期：仍保持橙红预警边框与图标，区别于三态 idle |
| KPI「已逾期」占位 | 无可解析计划完成日时显示 `—`，卡片 disabled，附「待同步」说明 |
| 一致性 | 环形图、状态 pill、成员进度辅文、控制点列表行遵循统一色板 |
...
| 10 | Scope pending 时锁死工作台 / 看板 / **进度看板** |
...
| 导航 Tab | **进度看板** |
| 列表状态概述 | 与看板环形图三态一致；列表为横条、看板为饼图；Pending 时「待 Scope」 |
| 进度 KPI | 未开始 / 测试中 / 已完成 / 已逾期 |
...
5. **进度看板**：筛选 ITA 组 → 查看 KPI / 需关注事项 → 抽屉双状态 → 跳转工作台
6. （有阻塞时）迷你依赖图展示阻塞链
