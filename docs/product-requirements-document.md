...
项目组**全员**可在独立「**进度看板**」Tab **只读**查看当前项目的控制点健康度、逾期风险，以及 Audit vs Specialist 分组进度。**控制点状态、节点进度、状态概述、组内成员进度** 均来自 `workspaceProgressService` 工作台 snapshot；看板 `task.status` **不在进度看板抽屉展示**（v1.6.10）。**不需要在进度看板内拖拽或编辑**。

**权限**：本期不做角色级差异——能访问该项目的成员均可查看进度看板 Tab。

### 13.2 页面结构（v1.6.10 · 摘要仪表盘 + 明细）

| 区块 | 内容 |
|------|------|
| 负责组筛选 | 全部 / Audit 主责 / ITA 组 / Tax 组 / FRM 组（联动 KPI、环形图、需关注、列表；**不**受负责人筛选影响） |
| **摘要 KPI 四格** | 未开始 / 测试中 / 已完成 / 已逾期（均来自控制点 `workspaceStatus` 与计划完成日；逾期待上游同步时显示占位）；**纯展示，不可点击**（v1.6.9） |
| **状态概述** | **环形图（三态）** + 图例；Tab：**全部 / GITC / ITAC**（v1.6.10）；中心为控制点总数；**图例纯展示** |
| **近期动态** | 按 `updatedAt` 展示最近保存的测试点（当前节点、TOD/TOE、**负责人彩色标签**） |
| **底部双卡** | 左：**组内成员进度**（§13.11）；右：**控制点节点进度**（§13.13） |
| **视觉语义** | 未开始=琥珀、测试中=蓝、已完成=绿；逾期=红；KPI / 环形图 / 图例 / pill **同色同源**（§13.12） |
| 需关注事项 | **计划逾期**、**长期未开始**（见 §13.3）；负责人以彩色标签展示 |
| 控制点列表 | Tab：**全部 · N** / GITC · N / ITAC · N / 其他（如有）；**负责人筛选**（§13.14）；列见 §13.4 |
| 只读抽屉 | **控制点状态** + **节点进度**（与列表一致）+ 工作台摘要 + 材料；**铺满**右侧列高度（§13.10） |
...
- **负责人筛选**下拉：与「控制点节点进度」卡片联动（§13.14）；选项含全部成员、当前负责组名册、未分配  
- 列：**控制点** / **负责人**（彩色 pill）/ **负责组** / **控制点状态** / **节点进度**  
- **控制点状态**列：主展示 `workspaceStatus`（三态 pill）；`progressStatus` 与三态不一致且非 blocked 时以小字补充  
- **节点进度**列（与抽屉一致）：  
...
KPI 四格与状态概述图例**仅作鸟瞰展示**，不触发列表筛选或页面布局联动（v1.6.9）；数据**仅来自工作台 snapshot**，不读取看板执行状态。

**KPI 卡片布局（v1.6.8）**：图标 | 标题+说明 | 大号数字；有计数时启用对应色强调（`has-value`），零计数为 idle 样式；无 hover 上浮与选中态（v1.6.9）。
...
| 列表列名 | 控制点 / 负责人 / 负责组 / **控制点状态** / **节点进度** |
| 负责人展示 | 全页 **彩色 pill 标签**（同一邮箱固定配色，§13.14） |
| 负责人筛选 | **负责人筛选** 下拉 + **清除**（列表与节点进度卡片联动） |

### 13.10 抽屉（v1.6.10 · 精简 · 铺满）

选中控制点后，右侧抽屉与左侧列表**等高铺满**；内容区可滚动，底部操作按钮固定。

| 区块 | 内容 |
|------|------|
| 标题区 | 类型标签、标题、ID、**负责人彩色标签** |
| 状态区 | **控制点状态**：`workspaceStatus` 三态 pill；细态差异时小字补充 |
...
| Team 标签 | **无**（跟随页顶负责组筛选） |
| 「全部」 | 各组执行层成员（IC + Staff）去重及其名下控制点 |
| 选定某组 | 该组执行层成员 + 对应 `contributorGroup` 控制点 |
| 成员行 | 成员名以 **负责人彩色标签** 展示；按控制点总数降序 |
| 柱图 | 填充 = 已完成占比；辅文为三态明细 |
| 未分配 | `owner` 为空时单独一行 |

### 13.13 控制点节点进度卡片（v1.6.10）

原「控制类型分布」改为按**控制点**展示节点完成度：

| 项 | 说明 |
|----|------|
| Tab | 全部 / GITC / ITAC |
| 每行 | 类型标签、控制点标题、**负责人彩色标签**、`completed/total · percent%`、进度条 |
...
| 未分配 | 灰色 pill |
| 覆盖范围 | 列表负责人列、抽屉标题、近期动态、需关注事项、组内成员进度、控制点节点进度 |
| 负责人筛选 | 两处入口：**控制点列表**表头、**控制点节点进度**卡片；共享 `ownerFilter` 状态 |
| 筛选影响 | 控制点列表、控制点节点进度卡片、近期动态（`detailControls`） |
| 筛选不影响 | KPI 四格、状态概述环形图、组内成员进度、需关注事项（仍仅受页顶负责组筛选） |
...
| 层级 | 规则 |
|------|------|
| 三态主色（全页统一） | `workspaceStatus`：**未开始=琥珀 `#ff991f`** / 测试中=蓝 `#0052cc` / 已完成=绿 `#00875a`；KPI 四格、环形图、图例色块、pill 共用 `progressVisualTokens.js`（v1.6.9） |
...
| 交互 | KPI 四格与状态概述图例**不可点击**；页顶负责组 chip + **负责人筛选**（列表/节点进度卡片，§13.14） |
...
| 19 | 进度抽屉展示 **控制点状态 + 节点进度**（v1.6.10）；看板执行状态不在抽屉展示 |
| 20 | 阻塞链预览**仅在有阻塞时**展示 |
...
5. **进度看板**：筛选 ITA 组 → 查看 KPI / 需关注事项 → **全部/GITC/ITAC** 标签 → **负责人筛选** → 抽屉节点进度 → 跳转工作台
6. （有阻塞时）迷你依赖图展示阻塞链
