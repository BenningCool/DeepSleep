...
- 计划开始日期 / 报告日（如有）
- **控制点数量**、成员数量（v1.6.11）
- **状态概述柱**（v1.6.4 / v1.6.11）：与进度看板 **状态概述环形图同一数据**（`workspaceStatus` 三态）；列表为 **横向进度柱**（仅填充测试中/已完成），看板为 **环形图**
...
|------|------|
| 0 控制点 | 灰空轨 +「暂无控制点」 |
| 有控制点，**全部未开始** | 灰空轨 + `{total} 控制点` + `未开始 n · 测试中 0 · 已完成 0`（**不填充琥珀色**，v1.6.11） |
| 有控制点，存在测试中/已完成 | 柱体仅填充 **测试中（蓝）** 与 **已完成（绿）** 段；未开始部分留空 |
...
| 21 | **v1.6.11**：控制点清单在工作台维护；概览 Scope 导入与三 Tab 门禁已移除 |
...
| 导航 Tab | **进度看板** |
| 列表状态概述 | 与看板环形图三态计数一致；列表柱**仅填测试中/已完成**；全未开始=空心灰轨 |
